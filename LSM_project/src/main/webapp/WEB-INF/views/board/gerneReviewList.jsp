	...
	<div id="s">
		...
		<div id="pg">
			...
			<c:if test="${startPage > 5 }">
				<a class="paging" href="/board/reviewList?pg=1">[맨앞]</a>
				<a class="paging" href="/board/reviewList?pg=${startPage - 1}">[이전]</a>
			</c:if>
			...
			<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
				...
				<c:if test="${i != pg }">
					<a class="paging" href="/board/reviewList?pg=${i}">[${i}]</a>
				</c:if>
				
			</c:forEach>

			<c:if test="${endPage < totalPage }">
				<a class="paging" href="/board/reviewList?pg=${endPage + 1}">[다음]</a>
				<a class="paging" href="/board/reviewList?pg=${totalPage}">[맨끝]</a>
			</c:if>
		</div>
